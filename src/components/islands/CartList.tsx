    ...
    await Swal.fire({
      ...
      html: `
        <div style="padding: 1rem;">
          <p style="font-size: 1.125rem; margin-bottom: 1rem; color: #4b5563;">Tu pedido ha sido procesado exitosamente.</p>
          <p style="font-size: 1.875rem; font-weight: bold; color: #10b981; margin-top: 1rem;">Total: $${totalPrice.toFixed(2)}</p>
        </div>
      `,
      icon: 'success',
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center p-6 opacity-60">
        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="mb-4 text-base-content/30"><circle cx="8" cy="21" r="1"/><circle cx="19" cy="21" r="1"/><path d="M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57l1.65-7.43H5.12"/></svg>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-4 p-2 max-h-[60vh] overflow-y-auto">
        {items.map((item) => (
          <div key={item.id} className="flex gap-4 p-3 bg-base-100/50 hover:bg-base-100 rounded-xl border border-base-content/5 transition-all duration-200 group">
            {/* Image */}
            ...
            <div className="flex-grow flex flex-col justify-between">
              <div className="flex justify-between items-start">
                  ...
                  <button
                      ...
                      className="text-error/50 hover:text-error transition-colors p-1 cursor-pointer"
                      aria-label="Eliminar"
                  >
              ...
              <div className="flex justify-between items-end mt-2">
                  <div className="flex items-center gap-2 bg-base-200/50 rounded-lg p-1">
                      <button
                          onClick={() => removeCartItem(item.id)}
                          className="btn btn-xs btn-ghost btn-square w-6 h-6 cursor-pointer"
      ...
      <div className="border-t border-base-content/10 pt-4 px-2 space-y-3">
